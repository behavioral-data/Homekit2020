...
BASE_COMMAND="--config configs/models/CNNToTransformerClassifier.yaml --model.num_hidden_layers 0 --model.dropout_rate 0.4  --model.learning_rate 0.003  --data.test_path $TEST_PATH --data.train_path $TRAIN_PATH --data.val_path $EVAL_PATH --model.batch_size 800 --trainer.check_val_every_n_epoch 1 --trainer.max_epochs 50 --trainer.log_every_n_steps 50 --early_stopping_patience 40 --model.warmup_steps 100"
