...
BASE_COMMAND="--config configs/models/InceptionTime.yaml  --model.val_bootstraps=0 --model.learning_rate 0.003 --data.test_path $TEST_PATH --data.train_path $TRAIN_PATH --data.val_path $EVAL_PATH --model.batch_size 125 --trainer.check_val_every_n_epoch 1 --trainer.max_epochs 50 --trainer.log_every_n_steps 50 --early_stopping_patience 5 --checkpoint_metric  val/roc_auc "
...
for i in ${!TASKS[*]};
  do
    for seed in ${!SEEDS[*]};
    do
      EXPERIMENT_NAME="InceptionTime-${TASKS[$i]}-temp_split-seed_${seed}-$(date +%F)"
