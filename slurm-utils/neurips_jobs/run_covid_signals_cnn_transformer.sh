...
BASE_COMMAND="--config configs/models/CNNToTransformerClassifier.yaml --model.learning_rate 5e-4  --data.train_path $TRAIN_PATH --data.val_path $EVAL_PATH --model.batch_size 800 --trainer.check_val_every_n_epoch 2 --trainer.max_epochs 200 --trainer.log_every_n_steps 50 --early_stopping_patience 10"
...
for i in ${!TASKS[*]}; 
  do
    ...
    eval "python slurm-utils/launch_on_slurm.py  -n 1 -m '64G' --num-gpus 1 --num-cpus 10 --dir . --exp-name ${TASKS[$i]} --command \"$pythonCommand\""
  done
