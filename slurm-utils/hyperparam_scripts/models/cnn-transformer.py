...
def train_fn(config,checkpoint_dir=None):
    dataset_args = validate_dataset_args(None,None,"src/data/dataset_configs/PredictTrigger.yaml")
...
def main():
    ray.init()
    obj_ref = ray.put(pickle.load(open("/homes/gws/mikeam/seattleflustudy/data/processed/cached_datareaders/PredictTrigger-train_eval.pickle", "rb" )))
    analysis = tune.run(
        train_fn,
        num_samples=3,
        config={
