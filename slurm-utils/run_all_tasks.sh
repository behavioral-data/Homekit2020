...
BASE_COMMAND="python src train-cnn-transformer --train_batch_size 700 --dropout_rate=0.4 --focal_gamma=1.7188155817156203 --learning_rate=5e-5 --n_epochs=30 --warmup_steps=20 --val_epochs 2 --model_config model_configs/small_embedding.yaml --train_path $TRAIN_PATH --eval_path $EVAL_PATH --test_path $TEST_PATH"
TASKS=(
    "PredictFluPos",
    "PredictFluSymptoms",
...
for task in ${TASKS[*]} 
  do
    ...
    eval "python slurm-utils/launch_on_slurm.py  -n 1 --num-gpus 2 --num-cpus 16 --dir . --exp-name $task --command \"$pythonCommand\""
  done
